...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const swiper = document.querySelector('.swiper2');
  const slides = swiper.querySelectorAll('.swiper-slide');

  const HIDDEN_COUNT = 10;
  let isOpened = false;

  // последние 8 слайдов
  const hiddenSlides = Array.from(slides).slice(-HIDDEN_COUNT);
  ...
  openBtn.addEventListener('click', (e) => {
    ...
    hiddenSlides.forEach(slide => {
      slide.classList.toggle('is-hidden', !isOpened);
    });

    // текст кнопки
    openBtn.textContent = isOpened
      ? 'Скрыть'
      : 'Показать все специальности';

    // добавляем / убираем класс show
    openBtn.classList.toggle('show', isOpened);
